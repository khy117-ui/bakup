...
$st = db()->prepare("SELECT * FROM hawbs WHERE $w ORDER BY on_board_date DESC, id DESC LIMIT $per OFFSET $off");
$st->execute($params);
$rows = $st->fetchAll();
...
<div class="head">
  ...
  <div class="right"><a class="btn pri" href="?p=hawb_form">새 HAWB</a></div>
</div>
...
<div class="card">
  ...
  <table>
    <thead><tr>
      ...
      <th style="width:110px">Flight / 목적지</th>
    ...
    <?php foreach ($rows as $r): ?>
      <tr>
        ...
        <td style="font-size:11.5px"><?= h(trim((string)$r['flight_no'] . ' ' . (string)$r['destination'])) ?></td>
        <td class="c">
